...
> **⚠️ 研究工具 (Research Utility)**
> 本目录包含用于模型微调的训练脚本，**独立于** AirLLM 核心推理库。
> 这些脚本使用 HuggingFace Transformers **在本地** 进行训练，不调用任何云端 API。
> 训练完成后，可使用 AirLLM 核心库进行推理：`from air_llm.airllm import AutoModel`
>
> **兼容性**：Python ≥ 3.10 · 可微调的模型包括 LLaMA、Qwen 3、Gemma 4 等
