...
"""QLoRA DPO (Direct Preference Optimization) training script.

This is a research/training utility, separate from the core AirLLM inference
library. It implements DPO loss with QLoRA for low-cost RLHF training of
large language models on a single GPU.

COMPATIBILITY NOTE (transformers ≥4.48.0):
- ``load_in_4bit`` / ``load_in_8bit`` kwargs on ``from_pretrained()`` are
  deprecated but still functional. The ``quantization_config`` parameter
  (also provided here) is the canonical approach.
- ``LlamaTokenizer`` is deprecated in favor of ``AutoTokenizer``, which
  handles Llama models natively.
- ``tokenizer_type='llama'`` kwarg on ``AutoTokenizer.from_pretrained()``
  is deprecated — ``AutoTokenizer`` auto-detects the correct class.
"""
...
logging_file_path = "./qlora_dpo_logs.log"
...
class ModelArguments:
    model_name_or_path: str | None = field(default="EleutherAI/pythia-12b")
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    mmlu_split: str | None = field(default="eval", metadata={"help": "The MMLU split to run on"})
    mmlu_dataset: str | None = field(
        default="mmlu-fs",
        metadata={
            "help": "MMLU dataset to use: options are `mmlu-zs` for zero-shot or `mmlu-fs` for few shot."
        },
    )
    do_mmlu_eval: bool | None = field(
        default=False, metadata={"help": "Whether to run the MMLU evaluation."}
    )
    max_mmlu_samples: int | None = field(
        default=None,
    ...
    report_to: str = field(
        default="none", metadata={"help": "To use wandb or something else for reporting."}
    )
    ...
    do_train: bool = field(
        default=True, metadata={"help": "To train or not to train, that is the question?"}
    )
    ...
    debug_mode: bool = field(
        default=False, metadata={"help": "debug mode sample 200 train/eval samples for validation"}
    )
    reference_model: str = field(
        default="timdettmers/qlora-hh-rlhf-7b",
        metadata={"help": "pretrained reference sft model name or path"},
    )
    reference_free: bool = field(
        default=False,
        metadata={
            "help": "If True, we ignore the _provided_ reference model and implicitly use a reference model that assigns equal probability to all responses."
        },
    )
    beta: float = field(
        default=0.1,
...
class SampleGenerateCallback(transformers.TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if "model" in kwargs:
            ...
            logger.info("model not found in kwargs, skipping")
...
class DataCollatorForCausalLM:
    ...
    def __call__(self, instances: Sequence[dict]) -> dict[str, torch.Tensor]:
        # Extract elements
        chosen = [
            f"{self.tokenizer.bos_token}{example['chosen']}{self.tokenizer.eos_token}"
            for example in instances
        ]
        rejected = [
            f"{self.tokenizer.bos_token}{example['rejected']}{self.tokenizer.eos_token}"
            for example in instances
        ]
        ...
        all_input_ids = pad_sequence(
            tokenized_input_ids_list, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )

        data_dict = {
            "chosen_input_ids": all_input_ids[: len(instances)],
            "chosen_attention_mask": all_input_ids[: len(instances)].ne(
                self.tokenizer.pad_token_id
            ),
            "rejected_input_ids": all_input_ids[len(instances) :],
            "rejected_attention_mask": all_input_ids[len(instances) :].ne(
                self.tokenizer.pad_token_id
            ),
            "return_loss": True,
        }
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> dict:
    ...
    def format_dataset(dataset, dataset_format):
        if dataset_format == "hh-rlhf" or (dataset_format is None and args.dataset == "hh-rlhf"):
            dataset = dataset.map(lambda x: {"rejected": x["rejected"], "chosen": x["chosen"]})
    ...
    if args.do_eval or args.do_predict:
        ...
        if args.group_by_length:
            eval_dataset = eval_dataset.map(
                lambda x: {"length": len(x["chosen"]) + len(x["rejected"])}
            )
...
def dpo_loss(
    ...
    Implements the standard DPO loss from Rafailov et al. (2023):
    ...
    This implementation is mathematically correct:
        pi_logratios = policy_chosen - policy_rejected   (policy log-ratio)
        ref_logratios = ref_chosen - ref_rejected         (reference log-ratio)
        logits = pi_logratios - ref_logratios             (advantage)
        loss = -logsigmoid(beta * logits)                 (DPO loss)
...
def compute_metrics(ep: EvalPrediction):
    ...
    return {
        "chosen_rewards": ep.predictions[0].mean(),
        "rejected_rewards": ep.predictions[1].mean(),
    }
...
def train():
    ...
    for k, v in dtypes.items():
        total += v
    for k, v in dtypes.items():
        logger.info("dtype %s: %d params (%.2f%%)", k, v, 100 * v / total)
