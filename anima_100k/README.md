# Anima 100K — 长上下文研究数据集与训练工具

> **⚠️ 研究数据集与训练工具 (Research Dataset & Training Utility)**
> 本目录包含 Anima 100K 长上下文训练相关的数据集、模型代码和训练脚本，
> **独立于** AirLLM 核心推理库。
> 100% 本地训练 — 不调用任何云端 API。Python ≥ 3.10。
> 训练完成后，可使用 AirLLM 核心库进行推理：`from air_llm.airllm import AutoModel`
>
> **与 AirLLM v3.0 的关系**：这些脚本最初为 Anima 项目编写，
> 与 AirLLM v3.0.0 架构保持兼容。AirLLM 核心库可用于训练后的模型推理。
