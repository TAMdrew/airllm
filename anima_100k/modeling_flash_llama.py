...
"""Custom Flash Attention LLaMA model for 100K context length.

This is a research/training utility, separate from the core AirLLM inference
library. It provides a modified LLaMA implementation with:
- FlashAttention v2 for O(seq_len * block_c) memory instead of O(seq_len^2)
- Flash RoPE (rotary position embeddings) with linear scaling
- XEntropy for memory-efficient cross-entropy loss computation
- Grouped-Query Attention (GQA) support

This module is used by the Anima 100K training pipeline to enable training
with sequence lengths up to 100K tokens on consumer GPU hardware.

NOTE: The print() statements in the import blocks below are intentional —
they provide immediate feedback about which Flash Attention components are
...
try:
    ...
    raise ImportError(
        "Please install xentropy kernels: `pip install git+https://github.com/HazyResearch/flash-attention.git#subdirectory=csrc/xentropy`"
    )
...
class LlamaAttention(nn.Module):
    ...
    def forward(
            ...
            if new_len > past_kv.size(1):
                past_kv = torch.cat(
                    [
                        past_kv,
                        torch.empty(
                            bsz, 256, 2, kv.size(3), kv.size(4), dtype=kv.dtype, device=kv.device
                        ),
                    ],
                    1,
                )
        ...
        if is_padded_inputs:
            ...
            attn_output = pad_input(attn_output, indices_q, bsz, max_seqlen_q).reshape(
                bsz, q_len, h_size
            )
